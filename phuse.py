...
def depr(major, minor, cause, fix):
    text = "Warning: Use of deprecated feature or API. (Deprecated in Phuse-%d.%d)\n"\
           "Cause: %s\n"\
...
class PhuseException(Exception):
    """ A base class for exceptions used by phuse. """
    pass
...
class RouteError(PhuseException):
    """ This is a base class for all routing related exceptions """


class RouteReset(PhuseException):
...
class Route(object):
    ...
    def __init__(self, app, rule, method, callback,
        ...
        #: A list of route-specific plugins (see :meth:`Phuse.route`).
        self.plugins = plugins or []
        #: A list of plugins to not apply to this route (see :meth:`Phuse.route`).
        self.skiplist = skiplist or []
        #: Additional keyword arguments passed to the :meth:`Phuse.route`
...
class Phuse(object):
    """ Each Phuse object represents a single, distinct web application and
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.get('catchall') is False:
            depr(0, 13, "Phuse(catchall) keyword argument.",
        ...
        if kwargs.get('autojson') is False:
            depr(0, 13, "Phuse(autojson) keyword argument.",
    ...
    def add_hook(self, name, func):
        """ Attach a callback to a hook. Three hooks are currently implemented:
            ...
            app_reset
                Called whenever :meth:`Phuse.reset` is called.
        """
    ...
    def _mount_app(self, prefix, app, **options):
        ...
        if options:
            depr(0, 13, "Unsupported mount options. Falling back to WSGI mount.",
                 "Do not specify any route options when mounting phuse application.")
            return self._mount_wsgi(prefix, app, **options)
    ...
    def mount(self, prefix, app, **options):
        """ Mount an application (:class:`Phuse` or plain WSGI) to a specific
            URL prefix. Example::

                parent_app.mount('/prefix/', child_app)

            :param prefix: path prefix or `mount-point`.
            :param app: an instance of :class:`Phuse` or a WSGI application.
            ...
            (:class:`Phuse` childs only), it is highly discouraged.
        ...
        if isinstance(app, Phuse):
            return self._mount_app(prefix, app, **options)
        else:
            return self._mount_wsgi(prefix, app, **options)

    def merge(self, routes):
        """ Merge the routes of another :class:`Phuse` application or a list of
            :class:`Route` objects into this application. The routes keep their
            'owner', meaning that the :data:`Route.app` attribute is not
            changed. """
        if isinstance(routes, Phuse):
    ...
    def _handle(self, environ):
        path = environ['phuse.raw_path'] = environ['PATH_INFO']
        if py3k:
            environ['PATH_INFO'] = path.encode('latin1').decode('utf8', 'ignore')

        environ['phuse.app'] = self
        ...
        try:
            ...
            environ['phuse.exc_info'] = sys.exc_info()
    ...
    def wsgi(self, environ, start_response):
        ...
        try:
            ...
            or environ['REQUEST_METHOD'] == 'HEAD':
                ...
                out = []
            exc_info = environ.get('phuse.exc_info')
            if exc_info is not None:
                del environ['phuse.exc_info']
    ...
    def __call__(self, environ, start_response):
        """ Each instance of :class:'Phuse' is a WSGI application. """
...
class BaseRequest(object):
    """ A wrapper for WSGI environment dictionaries that adds a lot of
        ...
        dictionary (as 'phuse.request.ext.<name>'). This is the recommended
    ...
    def __init__(self, environ=None):
        ...
        self.environ['phuse.request'] = self

    @DictProperty('environ', 'phuse.app', read_only=True)
    def app(self):
        """ Phuse application handling this request. """
        raise RuntimeError('This request is not connected to an application.')

    @DictProperty('environ', 'phuse.route', read_only=True)
    def route(self):
        """ The phuse :class:`Route` object that matches this request. """
    ...
    @DictProperty('environ', 'phuse.request.headers', read_only=True)
    def headers(self):
    ...
    @DictProperty('environ', 'phuse.request.cookies', read_only=True)
    def cookies(self):
    ...
    @DictProperty('environ', 'phuse.request.query', read_only=True)
    def query(self):
        """ The :attr:`query_string` parsed into a :class:`FormsDict`. These
            values are sometimes called "URL arguments" or "GET parameters", but
            not to be confused with "URL wildcards" as they are provided by the
            :class:`Router`. """
        get = self.environ['phuse.get'] = FormsDict()
        pairs = _parse_qsl(self.environ.get('QUERY_STRING', ''))
        for key, value in pairs:
            get[key] = value
        return get

    @DictProperty('environ', 'phuse.request.forms', read_only=True)
    def forms(self):
    ...
    @DictProperty('environ', 'phuse.request.params', read_only=True)
    def params(self):
    ...
    @DictProperty('environ', 'phuse.request.files', read_only=True)
    def files(self):
    ...
    @DictProperty('environ', 'phuse.request.json', read_only=True)
    def json(self):
    ...
    @DictProperty('environ', 'phuse.request.body', read_only=True)
    def _body(self):
        try:
    ...
    @DictProperty('environ', 'phuse.request.post', read_only=True)
    def POST(self):
    ...
    @DictProperty('environ', 'phuse.request.urlparts', read_only=True)
    def urlparts(self):
    ...
    def __setitem__(self, key, value):
        ...
        if self.environ.get('phuse.request.readonly'):
        ...
        for key in todelete:
            self.environ.pop('phuse.request.' + key, None)
    ...
    def __getattr__(self, name):
        ...
        try:
            var = self.environ['phuse.request.ext.%s' % name]
    ...
    def __setattr__(self, name, value):
        ...
        key = 'phuse.request.ext.%s' % name
        if hasattr(self, name):
            raise AttributeError("Attribute already defined: %s" % name)
        self.environ[key] = value
    def __delattr__(self, name):
        try:
            del self.environ['phuse.request.ext.%s' % name]
        except KeyError:
...
class BaseResponse(object):
    ...
    def set_cookie(self, name, value, secret=None, digestmod=hashlib.sha256, **options):
        """ Create a new cookie or replace an old one. If the `secret` parameter is
            ...
            support for it will be removed in later versions of phuse.
...
class HTTPResponse(Response, PhuseException):
    def __init__(self, body='', status=None, headers=None, **more_headers):
...
class PluginError(PhuseException):
    pass
...
class ConfigDict(dict):
    ...
    def load_config(self, filename, **options):
        """ Load values from an ``*.ini`` style config file.
            ...
            The special sections ``phuse`` and ``ROOT`` refer to the root
...
class AppStack(list):
    ...
    def push(self, value=None):
        """ Add a new :class:`Phuse` instance to the stack """
        if not isinstance(value, Phuse):
            value = Phuse()
        self.append(value)
        return value
    new_app = push
...
# Shortcuts for common Phuse methods.
...
def make_default_app_wrapper(name):
    ...
    @functools.wraps(getattr(Phuse, name))
    def wrapper(*a, **ka):
...
class FapwsServer(ServerAdapter):
    ...
    def run(self, handler):  # pragma: no cover
        ...
        if 'PHUSE_CHILD' in os.environ and not self.quiet:
...
class GeventServer(ServerAdapter):
    ...
    def run(self, handler):
        ...
        if not isinstance(threading.local(), local.local):
            msg = "Phuse requires gevent.monkey.patch_all() (before import)"
            raise RuntimeError(msg)
        if self.quiet:
            self.options['log'] = None
        address = (self.host, self.port)
        server = pywsgi.WSGIServer(address, handler, **self.options)
        if 'PHUSE_CHILD' in os.environ:
            import signal
...
class EventletServer(ServerAdapter):
    ...
    def run(self, handler):
        ...
        if not patcher.is_monkey_patched(os):
            msg = "Phuse requires eventlet.monkey_patch() (before import)"
            raise RuntimeError(msg)
        socket_args = {}
...
def load_app(target):
    """ Load a phuse application from a module and make sure that the import
...
def run(app=None,
    ...
    """ Start a server instance. This method blocks until the server terminates.
     ...
     """
    if NORUN: return
    if reloader and not os.environ.get('PHUSE_CHILD'):
        import subprocess
        fd, lockfile = tempfile.mkstemp(prefix='phuse.', suffix='.lock')
        environ = os.environ.copy()
        environ['PHUSE_CHILD'] = 'true'
        environ['PHUSE_LOCKFILE'] = lockfile
    ...
    try:
        ...
        if not server.quiet:
            _stderr("Phuse v%s server starting up (using %s)..." %
        ...
        if reloader:
            lockfile = os.environ.get('PHUSE_LOCKFILE')
...
class TemplateError(PhuseException):
    pass
...
%%except ImportError:
    <b>ImportError:</b> Could not generate the error page. Please add phuse to
    the import path.
%%end
""" % __name__
...
#: A thread-safe namespace. Not used by Phuse.
local = threading.local()

# Initialize app stack (create first empty Phuse app now deferred until needed)
# BC: 0.6.4 and needed for run()
apps = app = default_app = AppStack()

#: A virtual package that redirects import statements.
#: Example: ``import phuse.ext.sqlite`` actually imports `phuse_sqlite`.
ext = _ImportRedirect('phuse.ext' if __name__ == '__main__' else
                      __name__ + ".ext", 'phuse_%s').module
...
def _main(argv):  # pragma: no coverage
    ...
    if args.version:
        print('Phuse %s' % __version__)
        sys.exit(0)
    if not args.app:
        _cli_error("No application entry point specified.")

    sys.path.insert(0, '.')
    sys.modules.setdefault('phuse', sys.modules['__main__'])
